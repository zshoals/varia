...
static inline simd_iq fq_truncate_iq(simd_fq t)
{
		...
		float quadf[4];
		int32_t quad[4];
		varia_float32x4_store_unaligned(&quadf[0], t);
		...
		return varia_int32x4_intrin_load_unaligned(&quad[0]);
...
static inline simd_iq fq_round_iq(simd_fq t)
{
		...
		return vcvtq_s32_f32(varia_float32x4_add(t, varia_float32x4_load_all(0.5f)));
...
static inline simd_fq iq_convert_fq(simd_iq t)
{
		...
		int32_t quad[4];
		float quadf[4];
		varia_int32x4_store_unaligned(&quad[0], t);
		...
		return varia_float32x4_intrin_load_unaligned(&quadf[0]);
